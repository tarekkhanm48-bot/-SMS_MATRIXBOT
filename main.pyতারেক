...
def _detect_service_from_msg(msg: str) -> str:
    ...
    ml = msg.lower()
    # WhatsApp Business must be checked BEFORE plain WhatsApp
    if 'whatsapp business' in ml or 'wabiz' in ml:  return 'whatsapp_business'
...
def _send_otp_notifications(phone: str, raw_msg: str, sender: str, service_hint: str = ''):
    """
...
def otp_hook():
    ...
    try:
        ...
        _send_otp_notifications(phone, raw_msg, sender)

        # Track forwarded_stats for webhook panel
        if api_key:
            try:
                c.execute("SELECT name FROM sms_panels WHERE panel_type='webhook' AND value=?", (api_key,))
                wrow = c.fetchone()
                if wrow:
                    wname = wrow[0]; today_s = str(datetime.date.today())
                    c.execute(
                        "INSERT INTO forwarded_stats(panel_name,date,forwarded_count) VALUES(?,?,1) "
                        "ON CONFLICT(panel_name,date) DO UPDATE SET forwarded_count=forwarded_count+1",
                        (wname, today_s))
                    db.commit()
            except Exception:
                pass
                ...
                try:
                    ...
                    for rec in records:
                        ...
                        await loop.run_in_executor(
                            None, _send_otp_notifications, phone, msg, sender)
                        forwarded += 1
                    if forwarded:
                        print(f"[CRAPI] ✅ '{pname}' forwarded {forwarded} OTP(s)")
                        # ── Premium: track forwarded count per panel per day ──
                        today_s = datetime.date.today().isoformat()
                        c.execute(
                            "INSERT INTO forwarded_stats(panel_name,date,forwarded_count) "
                            "VALUES(?,?,?) ON CONFLICT(panel_name,date) DO UPDATE "
                            "SET forwarded_count=forwarded_count+?",
                            (pname, today_s, forwarded, forwarded))
                        db.commit()
                except Exception as e:
...
# Remove ivasms.com — blocked by Cloudflare (403)
c.execute("DELETE FROM sms_panels WHERE name='ivasms.com'")
# Clear stale seen_otps so all fresh OTPs are forwarded on restart
c.execute("DELETE FROM seen_otps")
...
def test_api_key(key: str) -> str:
    ...
    Returns a formatted result string showing which service accepts it.
    """
    key = key.strip()
    import base64 as _b64
    results = []
    ...
    for try_key in _unique([key, _try_decode64(key)]):
        ...
        try:
            ...
            if r.status_code == 200 and r.text.startswith("ACCESS_BALANCE:"):
                ...
                results.append(f"❌ sms-activate.org → {r.text[:60]}")
        except Exception as e:
    ...
    for try_key in _unique([key, _try_decode64(key)]):
        ...
        try:
            ...
            if r.status_code == 200 and "ACCESS_BALANCE" in r.text:
                ...
                results.append(f"❌ smshub.org → {r.text[:60]}")
        except Exception as e:
    ...
    return (f"❌ *Key NOT valid on any known service:*\n\n{fail_lines}\n\n"
            f"⚠️ This key does not work on 5sim, sms-activate, smshub, grizzlysms or onlinesim.\n"
            f"Please get a valid API key from your SMS panel account.")
...
async def on_msg(event):
    ...
    if state.startswith("pname_"):
        ptype = state[6:]   # fivesim / crapi / url
    ...
    if state.startswith("pval_"):
        ...
        if test_ok:
            ...
            type_labels = {"fivesim":"🔑 5sim","crapi":"📡 CRAPI","url":"🌐 URL"}
    ...
    if state == "keytest":
        await event.respond("⏳ Testing your key on 5 services... please wait.", parse_mode='md')
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, test_api_key, text)
        await event.respond(
            f"🔑 **Key Test Result**\n\n{result}",
            buttons=[[Button.inline("🔑 Test Another Key","adm_keytest")],
...
async def on_cb(event):
    ...
    if data == "adm_panels":
        ...
        def _panel_status(nm, pt, ecnt, dis_until, fwd_today):
        ...
        for pid, nm, pt, v, ecnt, dis_until, last_ok, last_err in pnls:
            ...
            icon, status_lbl = _panel_status(nm, pt, ecnt, dis_until, fwd_today)
        ...
        for pid, nm, pt, v, ecnt, dis_until, last_ok, last_err, icon, status_lbl, fwd_today in rows_info:
            pt_label = {"crapi":"CRAPI","fivesim":"5SIM","webhook":"WEBHOOK","url":"URL","apikey":"APIKEY"}.get(pt, pt.upper())
    ...
    if data == "pnl_live_status":
        ...
        for nm, pt, icon, status_msg, code in results:
            pt_label = {"crapi":"CRAPI","fivesim":"5SIM","webhook":"WH","url":"URL","apikey":"5SIM"}.get(pt, pt.upper())
            txt += f"{icon} **{nm}** `[{pt_label}]`\n"
            txt += f"   └ {status_msg}\n\n"
    ...
    if data.startswith("aptype_"):
        ptype = data[7:]
        labels = {"fivesim":"5sim API Key","crapi":"CRAPI URL","url":"JSON/URL Panel"}
        STATES[uid] = f"pname_{ptype}"
        await event.edit(
            f"📡 **{labels.get(ptype, ptype)}**\n\nPanel name পাঠান (e.g. 5sim-main):",
            buttons=[[Button.inline("🔙","adm_panels")]], parse_mode='md'); return
    ...
    if data.startswith("opn_"):
        ...
        type_label = {"crapi":"📡 CRAPI","fivesim":"🔑 5sim","url":"🌐 URL","webhook":"🪝 Webhook","apikey":"🔑 APIKey"}.get(pt, pt.upper())
    ...
    if data == "adm_keytest":
        ...
        await event.edit(
            "🔑 **API Key Tester**\n\n"
            "যেকোনো API key পাঠান — বট সাথে সাথে test করবে:\n"
            "• 5sim.net\n• sms-activate.org\n• smshub.org\n• grizzlysms.com\n• onlinesim.io\n\n"
            "⏳ Testing সাধারণত ৫-১৫ সেকেন্ড লাগে।",
