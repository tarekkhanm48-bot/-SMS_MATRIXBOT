...
def mask_number(phone: str) -> str:
    """Mask middle digits: +258872746205 → +2588XXXXXX205 (first 5 chars + XXXXXX + last 3)"""
    prefix = '+' if phone.startswith('+') else ''
    digits = re.sub(r'[^\d]', '', phone)
    full = prefix + digits
    if len(full) <= 8:
        return full
    return full[:5] + 'XXXXXX' + full[-3:]
...
def _send_otp_notifications(phone: str, raw_msg: str, sender: str):
    """Core OTP notification sender — used by webhook & panel poller."""
    ...
    c.execute("SELECT user_id FROM user_number_assignments "
              "WHERE REPLACE(REPLACE(number,'+',''),'-','')=?", (norm,))
    owner_row = c.fetchone()
    uid_owner = owner_row[0] if owner_row else None

    c.execute("SELECT country, service FROM premium_stock "
              "WHERE REPLACE(REPLACE(number,'+',''),'-','')=?", (norm,))
    ...
    # ── Private message to number owner (FULL number) ───────────
    if uid_owner:
        ...
        try:
            http_requests.post(tg_url, json={
                "chat_id": uid_owner, "text": priv_txt, "parse_mode": "Markdown"
            }, timeout=5)
        except: pass

    db.commit()

    # ── Public OTP Group (MASKED number) ────────────────────────
    comm_link = glink('support_group', 'https://t.me/SMS_MatrixBOT_OTP')
    group_txt = (
        f"🎉 *NEW OTP RECEIVED* 🎉\n\n"
        f"🌐 *Country:* {country_name}{country_flag}\n"
        f"📱 *Number:* `{masked}`\n"
    ...
    group_kbd = {"inline_keyboard": [[
        ...
        {"text": "🗄 Get Number", "url": f"https://t.me/{BOT_TOKEN.split(':')[0]}"}
    ]]}
    try:
        http_requests.post(tg_url, json={
            "chat_id": OTP_GROUP, "text": group_txt,
            "parse_mode": "Markdown", "reply_markup": group_kbd
        }, timeout=5)
    except: pass
...
def otp_hook():
    try:
        ...
        if not raw_msg or not phone: return "no msg", 400
...
async def poll_crapi_panels():
    """Background task: polls all CRAPI-type panels every 30 s."""
...
# Add CRAPI panel on startup
c.execute("INSERT OR IGNORE INTO sms_panels(name,panel_type,value,added_at) VALUES(?,?,?,?)",
          ("SMS-Matrix-Panel", "crapi",
           "http://198.135.52.36/crapi/ks/viewstats?token=Zh40PC-4wSbQoaYWge2fhQSd_SLzGreHV9WBPPHc8Vc&records=10",
...
async def on_msg(event):
    ...
    if state.startswith("pval_"):
        pname = state[5:]
        if 'crapi' in text.lower() or 'viewstats' in text.lower() or 'crapi' in pname.lower():
            ptype = "crapi"
...
async def main():
    await set_commands()
    asyncio.ensure_future(poll_crapi_panels())
